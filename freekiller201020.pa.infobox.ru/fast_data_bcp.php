					...
					<table>
						<thead>
							<tr class="row100 head">
								...
								<th class="cell100 column4"> Атакует </th>
								<th class="cell100 column5"> Защищается </th>
								<!-- <th class="cell100 column6"> Declared </th> -->
								<th class="cell100 column7"> Начало боя </th>
								<th class="cell100 column8"> Конец боя </th>
							</tr>
						</thead>
					</table>
				</div>
...
foreach ($attacks as $attack) {
    ...
    // foreach ($players2 as $pl2) {
        ...
        echo "<td class=\"cell100 column4\">{$name1}</td>";
        echo "<td class=\"cell100 column5\">{$name2}</td>";
        $dt1=CorrectDate($attack[4], 3*60*60);
        $dt2=CorrectDate($attack[5], 3*60*60);
        echo "<td class=\"cell100 column6\">{$dt1}</td>";
        // echo "<td class=\"cell100 column7\">{$dt2}</td>";
        if ($attack[6]==null) {
            if ($attack[4]>=date("Y-m-d H:i:s")) {
                echo "<td class=\"cell100 column8\"> В процессе </td>";
            } else {
                echo "<td class=\"cell100 column8\"> Предстоящий бой </td>";
            }
        } else {
            // echo "<td>{$attack[6]}</td>";
            $dt3=CorrectDate($attack[3], 3*60*60);
            echo "<td class=\"cell100 column8\"><p><a href=\"/fast_data2.php?from={$attack[0]}&to={$attack[1]}&attacker={$attack[2]}&defender={$attack[3]}&declared={$attack[4]}&resolved={$attack[5]}&end={$attack[6]}&clan_selected=$clan_selected\">{$dt3}</a></p></td>";
        }
        echo "</tr>";
        //     }
    }
